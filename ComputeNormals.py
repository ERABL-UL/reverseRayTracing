...
def uniformNormals(centroidsCoord_pnt, cornersCoord_pnt, ply_path_horiz):
    ...
    print("\n###################")
    print("Pointing normals toward the outside of each buildings ...")

    cloudFacade = ost.read_ply(ply_path_horiz)

    points_normals = np.c_[
        cloudFacade["x"], cloudFacade["y"], cloudFacade["z"], cloudFacade["nx"], cloudFacade["ny"], cloudFacade[
            "nz"]]  # No need for cloudFacade["z"]
    ...
    mask = []
    for i in tqdm(np.arange(points_normals.shape[0])):
        ...
        for j in np.arange(cornersCoord_pnt.shape[0]):
            ...
            if poly.contains(pnt_p) is True:
                mask.append(i)
                if np.sqrt(np.sum(np.square(pnt_plus_normal - centroidsCoord_pnt[j]))) < np.sqrt(
                        np.sum(np.square(pnt_xy - centroidsCoord_pnt[j]))):
